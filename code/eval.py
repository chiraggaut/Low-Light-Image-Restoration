...
def main():
    ...
    conf_path = os.path.join(root,args.opt)
    print(conf_path)
    input_dir_path= args.inputdir
    output_dir_path= args.outputdir
    high_dir_path= args.highdir
    ...
    lr_dir=os.path.join(root,input_dir_path)
    hr_dir=os.path.join(root,high_dir_path)

    out_dir=os.path.join(root,output_dir_path)
    ...
    for lr_path, hr_path, idx_test in zip(lr_paths, hr_paths, range(len(lr_paths))):
        ...
        with torch.cuda.amp.autocast():
            ...
            lr_t.cpu()
            sr_t = model.get_sr(lq=lq, heat=None)
            print('time taken in seconds: ', time.time()-t0)
        ...
        sr= cv2.LUT(np.array(sr), table)



        
        x= str(os.path.join(root,input_dir_path))+'/'
        n=len(x)
        y=str(lr_path)[n:]

        outfile=os.path.join(output_dir_path,y)
        path_out_sr=os.path.join(root,outfile)
